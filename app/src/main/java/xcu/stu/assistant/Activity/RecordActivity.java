...
public class RecordActivity extends Activity {
    ...
    private  void  initListener(){
        ...
        start.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {//如果未开启蓝牙，打开蓝牙设备
            ...
            public void onClick(View v) {
                ...
                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        try {
                            //遍历所有数据
                            for(BluetoothDevice device:students){
                                if(values==null){
                                    values = new ContentValues();
                                }
                                String select="select * from "+bean.getClassName()+" where " +
                                        ""+ClassAddActivity.NAME+"="+"'"+device.getName().split(":")[0]+"'";
                                Cursor cursor=database.rawQuery(select,null);
                                if(cursor.moveToNext()){//数据已添加
                                    int come=cursor.getInt(cursor.getColumnIndex(ClassAddActivity.COMED_NUM));
                                    come=come+1;
                                    values.put(ClassAddActivity.COMED_NUM,come);
                                    //更新数据
                                    database.update(bean.getClassName(),values,null,null);
                                }else {//第一次添加数据
                                    //插入新数据
                                    values.put(ClassAddActivity.NAME,device.getName().split(":")[0]);
                                    values.put(ClassAddActivity.NUM,device.getName().split(":")[1]);
    ...
    class  myReceiver extends BroadcastReceiver{
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(intent.getAction()== BluetoothDevice.ACTION_FOUND){
                ...
                //防止数据重复
                if (!isDeviceScaned(students,device)){
                    //判断是否是指定命名
    ...
    //判断设配是否已存在
    private  boolean  isDeviceScaned(ArrayList<BluetoothDevice>devices,BluetoothDevice device){
